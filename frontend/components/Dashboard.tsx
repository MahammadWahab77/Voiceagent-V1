...
const Dashboard: React.FC<DashboardProps> = ({ user, liveData, onLogout }) => {
    ...
    // State for Stages
    const [stages, setStages] = useState<Stage[]>([]); // Initialize empty, fetch on mount
    ...
    useEffect(() => {
        const fetchStagesAndProgress = async () => {
            try {
                // 1. Fetch All Stages
                const { data: stageConfigs, error: stageError } = await supabase
                    .from('stage_configs')
                    .select('*')
                    .order('stage_number');
                ...
                if (user?.id) { // Use user.id from props if available, or fetch using loose matching if needed
                    // Assuming user prop has the DB id. If not, we might need to lookup by mobile.
                    // For now, let's treat user.id as the student_id if it exists.
                    // Actually, in previous steps we saw user might just be {name, mobile}.
                    // Let's try to find student by mobile if ID is missing? 
                    // Or just default to 1 for now if we can't link.
                    // But wait, useGeminiLive.ts uses user.id. 
                }

                // Let's assume we can get the student details from the user object or local storage?
                // "user" prop comes from AuthPage. 
                // We should reload student status to be sure.

                if (stageConfigs) {
                    const mappedStages = stageConfigs.map((cfg: any) => ({
                        id: cfg.stage_number, // Use stage_number as UI ID for simplicity
                        title: cfg.name,
                        completed: false, // Will update below
                        active: false    // Will update below
                    }));

                    // Fetch current stage for this student
                    // We rely on the Prop `user` having an ID or Mobile.
                    let studentStage = 1;
                    if (user.mobileNumber) {
                        const { data: studentData } = await supabase
                            .from('students')
                            .select('current_stage')
                            .eq('mobile', user.mobileNumber) // Assuming unique mobile
                            .single();
                        if (studentData) {
                            studentStage = studentData.current_stage;
                        }
                    }

                    // Update Active/Completed Status
                    const updatedStages = mappedStages.map((s) => ({
                        ...s,
                        completed: s.id < studentStage,
                        active: s.id === studentStage
                    }));
                ...
                console.error("CRITICAL ERROR fetching stages:", err);
                // alert("Error loading stages. Please check console."); 
                // Do NOT fallback to old hardcoded stages, so we can see if it fails.
                // setStages([]); 
            }
        };
        ...
        // WebSocket already handles "STAGE_UPDATE" events to update UI, 
        // but the WebSocket handler in Dashboard.tsx needs to update THIS state.
    }, [user]);
                                    ...
                                    {transcripts.length === 0 ? (
                                        ...
                                        transcripts.map((item) => (
                                            <div key={item.id} className={`flex flex-col ${item.sender === 'user' ? 'items-end' : 'items-start'}`}>
                                                <div className={`max-w-[90%] p-2 rounded-xl text-xs ${item.sender === 'user'
                        ...
                        <div className={`w-full h-full glass-panel rounded-[24px] flex flex-col overflow-hidden relative transition-all duration-500 ${isOffline ? 'grayscale-[0.5] opacity-80' : ''}`}>
                            ...
                            >
                                {transcripts.length === 0 ? (
                                    ...
                                    transcripts.map((item) => (
                                        <div key={item.id} className={`flex flex-col ${item.sender === 'user' ? 'items-end' : 'items-start'}`}>
                                            <div className={`max-w-[85%] p-2 rounded-xl text-xs ${item.sender === 'user'
                    ...
                    {isConnected ? (
                        <>
                            ...
                            <button
                                ...
                                className={`w-14 h-14 md:w-16 md:h-16 flex items-center justify-center rounded-full transition-all duration-300 shadow-lg hover:scale-105 active:scale-95 border ${isMuted
                        ...
                        <button
                            ...
                            className={`w-16 h-16 md:w-20 md:h-20 flex items-center justify-center rounded-full transition-all duration-300 shadow-xl hover:scale-105 active:scale-95 disabled:opacity-50 disabled:scale-100 ring-4 ${isOffline
